...
df = pd.read_excel('base_preparada_racismo_ambiental_261mun.xlsx', sheet_name='Base_Preparada', header=1)
df['interacao_esgoto_raca'] = df['tx_esgoto_pct'] * df['prop_negra_parda_pct']
print(f"Base carregada: {df.shape[0]} municípios × {df.shape[1]} variáveis")

# ── 3. SELEÇÃO DE VARIÁVEIS ──────────────────────────────────
Y_COL  = 'tx_internacao_100k'
X_COLS = ['tx_esgoto_pct', 'prop_negra_parda_pct', 'interacao_esgoto_raca', 'regiao_cod', 'capital_cod', 'porte_cod']
...
coef_df = pd.DataFrame({
    ...
    'Interpretação': [
        ...
        'Interação entre saneamento e proporção negra/parda',
